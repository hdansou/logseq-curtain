...
- [ ] **T3.7** **Collect orphaned payloads.** Observed live: block 1058 kept `{"ma":"robots"}` after its macro was undone/edited away. Two problems — orphans accumulate unboundedly, and *undoing a concealment does not remove the text from storage*, so someone who conceals then undoes reasonably believes it is gone when it is not. Drop payload keys with no matching macro in the block title, on block change. The key-matching predicate is pure and testable.
