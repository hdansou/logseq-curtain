...
- [x] **T3.3f** Un-conceal — `Curtain: un-conceal fragments in this block`, so a concealed fragment can be edited normally. Writes the restored title *before* clearing payloads, the reverse of concealing and for the same reason: if the second step fails the text still exists somewhere.
