...
- [x] **T3.11** Self-healing round trip. Inline text goes back to the property on block change, **skipping the block being edited** — rewriting under the cursor would move it, and inline text is exactly what the user asked to see while editing. Parses the raw title, taking everything between the first and last comma as the text, so a comma survives where mldoc's lossy split would not. 9 tests.
